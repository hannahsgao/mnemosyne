...
test("fingerprinted release assets receive immutable browser caching", async () => {
  const request = new Request(
    "https://mnemosyne.example/catalog-data/v1/releases/fingerprint/series/horse.json",
  );
  let calls = 0;
  let assetPath = "";
  const response = await serveCatalogAsset(request, async (assetRequest) => {
    calls += 1;
  ...
  assert.equal(assetPath, "/data/v1/releases/fingerprint/series/horse.json");
  assert.ok(response);
...
test("mutable pointers are proxied without changing their revalidation policy", async () => {
  let calls = 0;
  let assetPath = "";
  const response = await serveCatalogAsset(
    new Request("https://mnemosyne.example/catalog-data/v1/manifest.json"),
    async (assetRequest) => {
      calls += 1;
      assetPath = new URL(assetRequest.url).pathname;
      return Response.json({}, {
  ...
  assert.equal(calls, 1);
  assert.equal(assetPath, "/data/v1/manifest.json");
  assert.equal(
...
test("non-catalog paths and non-read methods bypass the catalog asset binding", async () => {
  let calls = 0;
...
test("missing release assets retain the asset service error response", async () => {
  const response = await serveCatalogAsset(
    new Request(
      "https://mnemosyne.example/catalog-data/v1/releases/missing/series/horse.json",
    ),
