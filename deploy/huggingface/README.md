...
hf buckets sync \
  ...
  "hf://buckets/$MNEMOSYNE_HF_BUCKET_ID/$MNEMOSYNE_HF_RELEASE_PREFIX" \
  --dry-run
```
...
wrong or incomplete. The versioned prefix also leaves the previous Met-only
bundle at the bucket root for rollback.
    ...
    json.dumps(
        {
            ...
            "corpusCount": result["corpus"]["count"],
